...
<div class="box-body">
    ...
    <div class="products-form">
        ...
        <?= $form->field($model, 'product_logo')->fileInput() ?>
        <div id="previewimage">
        ...
        <div id="oldimage">
            ...
            if (!$model->isNewRecord) {
                if ($model->product_logo != 'no-image.jpg') {
                    ?>
                    <img src="/backend/web/uploads/product/<?php echo $model->product_id . '/' . $model->product_logo; ?>" height="75" width="100">
                <?php } else { ?>
        ...
        <?= $form->field($model, 'min_reorder')->textInput()->label('Minimum Reorder Level<span class="required-label"></span>'); ?>

        <?= $form->field($model, 'stock')->textInput(['maxlength' => true])->label('Stock<span class="required-label"></span>'); ?>

        <?php // echo $form->field($model, 'price_per_unit')->textInput(['maxlength' => true])->label('Price-per-unit<span class="required-label"></span>'); ?>
        <?php
        if ($model->isNewRecord) {
            $model->status = true;
        }
...
$script = <<< JS
   ...
   //Preview the image
         $('#previewimage').hide();
        $('#products-product_logo').on('change', function(e) {
         $('#oldimage').hide();
