...
class OrderstatusColumn extends DataColumn {
     ...
     * @var array
     */
    public $contentOptions = ['style' => 'text-align:left; vertical-align: middle;'];

    /**
     * @var array
     */
    public $headerOptions = ['style' => 'text-align:left;'];
    ...
    protected function setCellStyleOptions() {
        $this->contentOptions = ArrayHelper::merge(
                        ['style' => 'text-align:center; width:80px; white-space:nowrap;'], $this->contentOptions
        );
    }
    ...
    protected function checkOptionsArray() {
        ...
        if (empty($this->optionsArray)) {
            $this->optionsArray = [
                    [OrderStatus::OR_NEW, 'New Order', 'info'],
                    [OrderStatus::OR_INPROGRESS, 'In Progress', 'warning'],
                    [OrderStatus::OR_COMPLETED, 'Completed', 'success'],
                    [OrderStatus::OR_DISPATCHED, 'Dispatched', 'default'],
                    [OrderStatus::OR_DELEVERED, 'Delivered', 'success'],
                    [OrderStatus::OR_CANCELED, 'Cancelled', 'danger'],
            ];
        }
    }
